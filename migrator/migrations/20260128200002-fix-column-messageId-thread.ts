...
export const up: Migration = async ({ context: sequelize }) => {
		...
		for (const threadRecord of threadRecords) {
			try {
				...
				if (!message || !channelClientId || !userClientId) {
					console.log(
						`Message ${discordMessageId} not found in Discord, deleting thread record`,
					);
					// Discord上で既に削除されているデータのThreadレコードを削除する
					await sequelize.query(
						`DELETE FROM ${THREADS_TABLE_NAME} WHERE messageId = :messageId AND communityId = :communityId`,
						{
							replacements: {
	...
	await queryInterface.changeColumn(THREADS_TABLE_NAME, COLUMN_NAME_MESSAGE_ID, {
		type: DataTypes.INTEGER,
		allowNull: false,
	});
	console.log(
		`Changed ${COLUMN_NAME_MESSAGE_ID} to NOT NULL in ${THREADS_TABLE_NAME} table`,
	);
};
