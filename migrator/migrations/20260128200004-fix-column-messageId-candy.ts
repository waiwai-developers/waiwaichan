...
export const up: Migration = async ({ context: sequelize }) => {
	...
	// Step 1: CandiesテーブルのDiscordClientのmessageIdを全て取得しdiscordClientにアクセスし
	// CommunityテーブルやUsersテーブルやChannelsテーブルのidを取得しMessageテーブルのレコードとして発行する
	console.log(
		"Step 1: Fetching Candy records and creating Message records via Discord API...",
	);

	// Get all Candy records
	const candies = await sequelize.query<CandyRecord>(
		`SELECT communityId, messageId FROM ${CANDIES_TABLE_NAME}`,
		{ type: QueryTypes.SELECT },
	);

	console.log(`Found ${candies.length} Candy records`);
		...
		try {
			...
			for (const candy of candies) {
				try {
					...
					const guildClientId = communityById.get(candy.communityId);
					if (!guildClientId) {
						console.warn(
							`Community not found for communityId: ${candy.communityId}`,
						);
						continue;
					}
					...
					for (const [, channel] of textChannels) {
						try {
							...
							const message = await channel.messages.fetch(candy.messageId);
							if (message) {
					...
					if (!foundMessage) {
						console.warn(
							`Message not found for messageId: ${candy.messageId} in guild: ${guildClientId}`,
						);
						continue;
					}

					// Get channel ID from our database
					const channelKey = `${foundMessage.channelId}_${candy.communityId}`;
					const channelId = channelByClientIdAndCommunityId.get(channelKey);
					if (!channelId) {
						console.warn(
							`Channel not found in database for clientId: ${foundMessage.channelId}, communityId: ${candy.communityId}`,
						);
						continue;
					}

					// Get user ID from our database
					const userKey = `${foundMessage.author.id}_${candy.communityId}`;
					const userId = userByClientIdAndCommunityId.get(userKey);
					if (!userId) {
						console.warn(
							`User not found in database for clientId: ${foundMessage.author.id}, communityId: ${candy.communityId}`,
						);
						continue;
					}
					...
					messagesToInsert.push({
						...
						clientId: candy.messageId,
						communityId: candy.communityId,
						channelId: channelId,
						userId: userId,
					...
					console.error(
						`Error processing candy messageId: ${candy.messageId}`,
						error,
					);
				}
			}
	...
	// Step 2: CandiesテーブルのmessageIdをclientIdに命名を変更する
	// Candiesテーブルはidカラムがauto incrementでprimary keyなので、PRIMARY KEY制約の操作は不要
	console.log("Step 2: Renaming messageId to clientId in Candies table...");
	...
	// Step 3: Candiesテーブルに新たにmessageIdのカラムをIntegerで発行する
	console.log(
		"Step 3: Adding new messageId column (INTEGER) to Candies table...",
	);
	...
	// Step 4: CandiesテーブルのclientIdと一致するMessageテーブルのidを取得しCandiesテーブルのmessageIdを埋める
	console.log(
		"Step 4: Updating Candies.messageId with Messages.id based on clientId...",
	);
	await sequelize.query(`
		UPDATE ${CANDIES_TABLE_NAME} AS c
		INNER JOIN ${MESSAGES_TABLE_NAME} AS m ON c.${COLUMN_NAME_CLIENT_ID} = m.clientId AND c.communityId = m.communityId
		SET c.${COLUMN_NAME_MESSAGE_ID} = m.id
	`);

	// Step 5: CandiesテーブルのmessageIdにnot null 制約を追加する
	console.log("Step 5: Adding NOT NULL constraint to Candies.messageId...");
