...
export class PullRequestRepositoryImpl implements IPullRequestRepository {
    ...
    async getById(pr: GithubPullRequestId): Promise<PullRequestDto | undefined> {
        ...
        ).then((r) => this.toDto(r.data.user.login, r.data.state === "open", r.data.id));

    }

    toDto(ownerId:string,state:boolean,prId: number):PullRequestDto {
        return new PullRequestDto(new GitHubUserId(ownerId),new GithubPullRequestStatus(state),new GithubPullRequestId(prId))
    }

    async assignReviewer(user: GitHubUserId, pr: GithubPullRequestId): Promise<PullRequestDto | undefined> {
         return this.octokit.request(
        ...
        ).then((r) => this.toDto(r.data.user?.login ? r.data.user.login : "", r.data.state === "open", r.data.id));
    }
}
