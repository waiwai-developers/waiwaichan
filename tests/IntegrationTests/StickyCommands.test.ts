...
*/
	it("should send sticky message to channel with correct content", async function (this: Mocha.Context) {
		this.timeout(10_000);
		...
		// テスト用のスティッキーメッセージ
		const stickyMessage = "これはスティッキーメッセージです";
		...
		// createメソッドが成功メッセージを返すようにモック
		when(stickyLogicMock.create(anything())).thenResolve("スティッキーを登録したよ！っ");
		...
		// TextChannelインスタンスを作成
		// instanceofチェックを通過するためにTextChannelのプロトタイプを継承
		let sentMessage = "";
		const textChannelInstance = Object.create(TextChannel.prototype);
		textChannelInstance.type = "GUILD_TEXT";
		textChannelInstance.send = (message: string) => {
			sentMessage = message;
			return Promise.resolve({ id: "12345", content: message } as any);
		};
		...
		// モーダル送信のモック - スティッキーメッセージを送信
		const modalSubmitMock = {
			fields: {
				getTextInputValue: (id: string) => (id === "stickyInput" ? stickyMessage : null),
			},
			reply: (message: string) => {
				return Promise.resolve({} as any);
			},
		...
		expect(sentMessage).to.eq(stickyMessage);

		// StickyLogicのcreateメソッドが呼ばれたことを検証
		verify(stickyLogicMock.create(anything())).once();
