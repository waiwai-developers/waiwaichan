...
@injectable()
export class VoiceChannelDisconnectHandler implements VoiceChannelEventHandler<VoiceChannelState> {
	...
	async handle({ oldState, newState }: VoiceChannelState): Promise<void> {
		...
		const embed = new EmbedBuilder()
			...
			.addFields(
				{ name: '終了ユーザー', value: `<@${oldState.member.user.id}>`, inline: true },
				{ name: '終了時刻', value: new Date().toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo' }), inline: true }
			)
			.setColor(0xb80094)
