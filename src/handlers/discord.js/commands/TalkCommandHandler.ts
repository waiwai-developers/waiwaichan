...
export class TalkCommandHandler implements SlashCommandHandler {
	...
	async handle(
		...
		const personality = await this.personalityLogic.find(PersonalityId.PERSONALITY_ID_WAIWAICHAN)
		if (!personality) {
			return;
		}
		const personalityCategory = await this.personalityCategoryLogic.find(new PersonalityCategoryId(interaction.options.getInteger("id", true)), new PersonalityCategoryPersonalityId(personality.id.getValue()))
		...
		const metadata = Object.assign(personality.personality.getValue(), personalityCategory.context.getValue())
